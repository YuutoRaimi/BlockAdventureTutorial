...
public class GridManager : MonoBehaviour
{
    ...
    private void SpawnGridSquares()
    {
        ...
        for (var row = 0; row < rows; ++row)
        {
            for (var column = 0; column < columns; ++column)
            {
                ...
                _gridSquares[_gridSquares.Count - 1].GetComponent<GridSquare>().SetImage(_lineIndicator.GetSquareIndex(square_index) % 2 == 0);
                square_index++;
            }
        }
    }
    ...
    private bool CheckIfShapeCanBePlacedOnGrid(Shape currentShape)
    {
        ...
        if (currentShape.TotalSquareNumber != originalShapeFilledUpSquares.Count) Debug.LogError("Number of filled up squares are not the same as the original shape have.");
